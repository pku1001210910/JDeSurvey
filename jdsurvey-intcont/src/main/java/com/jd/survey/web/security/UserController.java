...
public class UserController {
	...
	public String createPost(@RequestParam(value = "_proceed", required = false) String proceed, @Validated({ User.UserInfo.class, User.Password.class }) User user,
	...
	public String list_internal(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Principal principal,
			Model uiModel) {
		try {
	...
	public String list_external(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Principal principal,
			Model uiModel) {
		try {
			int sizeNo = size == null ? DEFAULT_NUMBER_OR_RECORDS : size.intValue();
			final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
	...
	public String search_byName_post(@RequestParam(value = "_proceed", required = false) String proceed, @Validated({ User.UserSearchByName.class }) User user,
			BindingResult bindingResult, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		try {
			uiModel.addAttribute("searchType", "name");
			if (proceed != null) {
	...
	public String search_byLogin(@RequestParam(value = "_proceed", required = false) String proceed, @Validated({ User.UserSearchByLogin.class }) User user,
			BindingResult bindingResult, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		try {
			uiModel.addAttribute("searchType", "login");
			if (proceed != null) {
	...
	public String search_byEmail(@RequestParam(value = "_proceed", required = false) String proceed, @Validated({ User.UserSearchByEmail.class }) User user,
			BindingResult bindingResult, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
	...
	public String update(@RequestParam(value = "_proceed", required = false) String proceed, @Validated({ User.UserInfo.class }) User user, BindingResult bindingResult,
	...
	public String updatepassword(@RequestParam(value = "_proceed", required = false) String proceed, @Validated({ User.Password.class }) User user, BindingResult bindingResult,
			Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		try {
			if (proceed != null) {
				if (bindingResult.hasErrors()) {
	 ...
	 * @Secured({"ROLE_ADMIN"})
	 * 
	 * @RequestMapping( params = "export", produces = "text/html") public
	 * ModelAndView userReport(Principal principal, Model uiModel,
	 * HttpServletRequest httpServletRequest) { try{ Set<User> users =
	 * userService.user_findAll(); ModelAndView modelAndView =new
	 * ModelAndView("userList"); modelAndView.addObject("users",users); return
	 * modelAndView; } catch (Exception e) { log.error(e.getMessage(), e); throw
	...
	/**
	 * Helper method that handles the response in case of on an unhandled
	 * runtime error
	 * 
	 * @param pathSegment
	 * @param httpServletRequest
