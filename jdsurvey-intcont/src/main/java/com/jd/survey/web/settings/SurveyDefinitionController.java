...
public class SurveyDefinitionController {
	...
	// EXTERNAL_SITE_BASE_URL="external_site_base_url";
	...
	public void getSurveyQRCode(@PathVariable("id") Long surveyDefinitionId, Model uiModel, Principal principal, HttpServletRequest httpServletRequest,
		...
		try {
			...
			// String surveyLink
			// =messageSource.getMessage(EXTERNAL_SITE_BASE_URL, null,
	...
	public void getSurveyLogo(@PathVariable("id") Long surveyDefinitionId, Model uiModel, Principal principal, HttpServletRequest httpServletRequest,
			HttpServletResponse response) {
		try {
			uiModel.asMap().clear();
			User user = userService.user_findByLogin(principal.getName());
	...
	public String updateLogoPrepare(@RequestParam(value = "id", required = false) Long surveyDefinitionId, Model uiModel, Principal principal,
	...
	public String uploadFromJson(@RequestParam(value = "id", required = false) Long departmentId, Model uiModel, Principal principal, HttpServletRequest httpServletRequest) {
		try {
	...
	public String importTemplate(@RequestParam(value = "id", required = false) Long departmentId, @RequestParam(value = "templateId", required = false) Long templateId,
	...
	public String importTemplatePost(@RequestParam("id") Long departmentId, @RequestParam("secId") Long sectorId,
		...
		try {
			...
			if (proceed != null) {
				...
				return "redirect:/settings/surveyDefinitions/browsetemplate?id=" + encodeUrlPathSegment(departmentId.toString(), httpServletRequest) + "&sid="
	...
	public String browseTemplate(@RequestParam(value = "id", required = false) Long departmentId, @RequestParam(value = "sid", required = false) Long sectorId, Principal principal,
	...
	public String browseTemplatePost(@RequestParam(value = "id", required = false) Long departmentId, @RequestParam(value = "secId", required = false) Long sectorId,
	...
	public String saveTemplate(@PathVariable("id") Long surveyDefinitionId, Principal principal, Model uiModel, HttpServletRequest httpServletRequest,
			HttpServletResponse response) {
		try {
			String login = principal.getName();
			User user = userService.user_findByLogin(login);
	...
	public String saveTemplatePost(@RequestParam("sid") Long surveyDefinitionId, @RequestParam("secId") Long sectorId, @RequestParam("name") String templateName,
			@RequestParam("description") String templateDesc, @RequestParam(value = "_proceed", required = false) String proceed, Principal principal, Model uiModel,
	 ...
	 * migrate survey definitions from different environments, For example from
	...
	public String uploadFromJsonPost(@RequestParam("file") MultipartFile file, @RequestParam("id") Long departmentId, @RequestParam("name") String surveyName,
			@RequestParam(value = "_proceed", required = false) String proceed, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		try {
			String login = principal.getName();
			User user = userService.user_findByLogin(login);
	...
	public void exportToJson(@PathVariable("id") Long surveyDefinitionId, Principal principal, HttpServletRequest httpServletRequest, HttpServletResponse response) {
		try {
			String login = principal.getName();
			User user = userService.user_findByLogin(login);
			// Check if the user is authorized
	...
	/**
	 * Publishes the survey definition, this will make the survey available from
	 * the external site.
	 * 
	 * @param surveyDefinitionId
	 * @param response
	...
	public String publishSurveyDefinition(@PathVariable("id") Long surveyDefinitionId, HttpServletResponse response, Principal principal, Model uiModel,
			HttpServletRequest httpServletRequest) {
		try {
			String login = principal.getName();
			User user = userService.user_findByLogin(login);
	...
	public String DeactivateSurveyDefinition(@PathVariable("id") Long surveyDefinitionId, HttpServletResponse response, Principal principal, Model uiModel,
			HttpServletRequest httpServletRequest) {
	...
	public String createGet(@PathVariable("id") Long departmentId, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		...
		if (surveySettingsService.department_getCount() > 0) {
			try {
				...
				if (departmentId != null && departmentId != 0) {
					...
					Department department = surveySettingsService.department_findById(departmentId);
					surveyDefinition = new SurveyDefinition(department, surveySettingsService.velocityTemplate_findById(SURVEY_INVITATION_EMAIL_TEMPLATE_ID).getDefinition(),
	...
	public String createPost(@RequestParam(value = "_proceed", required = false) String proceed, @Valid SurveyDefinition surveyDefinition, BindingResult bindingResult,
		...
		try {
			...
			if (proceed != null) {
				...
				// bindingResult.rejectValue("autoRemindersWeeklyOccurrence",
				// "field_unique");
		...
		try {
			...
			for (SurveyDefinitionPage page : surveyDefinition.getPages()) {
				for (Question question : page.getQuestions()) {
					...
					// .addAttribute("datasetItems" + "p"+ page.getOrder() +
					// "q"+
	...
	public String list(@RequestParam(value = "s", required = false) Integer showPublishedSurveyDeleteFailMessage, @RequestParam(value = "page", required = false) Integer page,
	...
	public String update(@RequestParam(value = "_proceed", required = false) String proceed, @Valid SurveyDefinition surveyDefinition, BindingResult bindingResult,
			Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
	...
	public String delete(@PathVariable("id") Long id, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		...
		try {
			...
			if (surveyDefinition.getStatus().equals(SurveyDefinitionStatus.P) || surveyDefinition.getStatus().equals(SurveyDefinitionStatus.D)) {
				...
				log.warn("Attempt to delete a survey that has been published path:" + httpServletRequest.getPathInfo() + " attempted by user login:" + principal.getName()
	...
	/**
	 * Populates the model with the survey definition and the list of
	 * departments
	 * 
	 * @param uiModel
	 * @param surveyDefinition
