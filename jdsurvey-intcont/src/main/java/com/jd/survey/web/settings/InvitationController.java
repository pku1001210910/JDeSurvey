...
public class InvitationController {
	...
	// INTERNAL_SITE_BASE_URL="internal_site_base_url";
	// private static final String
	...
	/**
	 * Shows the list of Survey Invitations for a Survey Definition, Supports
	 * Paging
	 * 
	 * @param surveyId
	 * @param principal
	...
	public String listSurveyInvitations(@RequestParam(value = "id", required = true) Long surveyDefinitionId, @RequestParam(value = "page", required = false) Integer page,
			@RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "fileContentError", required = false) boolean contentError, Model uiModel,
		...
		try {
			...
			uiModel.addAttribute("fileContentError", contentError);// Used to
																	// carry
																	// error
																	// from file
																	// upload
	...
	public String prepareForImport(@RequestParam(value = "id", required = false) Long surveyDefinitionId, Principal principal, Model uiModel,
	...
	public String sendInvitations(@RequestParam("file") MultipartFile file, @RequestParam("id") Long surveyDefinitionId,
		...
		try {
			...
			if (proceed != null) {
				...
				// String trackingImageLink
				// =messageSource.getMessage(INTERNAL_SITE_BASE_URL, null,
				// LocaleContextHolder.getLocale());
				...
				if (!file.isEmpty() && ((file.getContentType().equalsIgnoreCase("application/vnd.ms-excel")) || (file.getContentType().equalsIgnoreCase("text/csv"))
						|| (file.getContentType().equalsIgnoreCase("text/plain")))) {
					CSVReader csvReader = new CSVReader(new InputStreamReader(file.getInputStream()));
					String[] nextLine;
					nextLine = csvReader.readNext(); // skip the first row the
					...
					while ((nextLine = csvReader.readNext()) != null) {
						...
						// Prevents IndexOutOfBoundException by skipping line if
						// there are not enough elements in the array nextLine.
						if (nextLine.length < 4) {
							// Continues if a blank line is present without
							// setting an error message (if a new line character
							// is present CSVReader will return and array with
							// one empty string at index 0).
						...
						// Prevents exception from attempting to send an email
						// with an empty string for an email address.
						if (nextLine[3].isEmpty()) {
							uiModel.addAttribute("fileContentError", true);
							continue;
						}
						// creating the Invitation
						Invitation invitation = new Invitation(nextLine[firstNameFieldIndex].trim(), nextLine[middleNameFieldIndex].trim(), nextLine[lastNameFieldIndex].trim(),
								nextLine[emailNameFieldIndex].trim(), surveyDefinition);

						Map model = new HashMap();
						// survey name
						model.put(messageSource.getMessage(SURVEY_NAME, null, LocaleContextHolder.getLocale()).replace("${", "").replace("}", ""), surveyDefinition.getName());
						// full name
						...
						model.put(messageSource.getMessage(INVITE_FILL_SURVEY_LINK_PARAMETER_NAME, null, LocaleContextHolder.getLocale()).replace("${", "").replace("}", ""),
								"<a href='" + surveyLink + surveyDefinition.getId() + "?list'>"
										+ messageSource.getMessage(INVITE_FILL_SURVEY_LINK_LABEL, null, LocaleContextHolder.getLocale()) + "</a>");
						...
						if (emailContent.length() > 14 && emailContent.substring(emailContent.length() - 14).toUpperCase().equalsIgnoreCase("</BODY></HTML>")) {
							...
							// template is incorrect or not html do not include
							// tracking white gif
