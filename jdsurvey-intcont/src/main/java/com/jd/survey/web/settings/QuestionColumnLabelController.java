...
public class QuestionColumnLabelController {
	...
	public String updateForm(@PathVariable("id") Long questionId, Principal principal, HttpServletRequest httpServletRequest, Model uiModel) {
	...
	public String createPost(Question question, BindingResult bindingResult, @RequestParam(value = "_proceed", required = false) String proceed, Principal principal, Model uiModel,
		...
		try {
			...
			if (!securityService.userIsAuthorizedToManageSurvey(surveySettingsService.question_findById(question.getId()).getPage().getSurveyDefinition().getId(), user)
					&& !securityService.userBelongsToDepartment(surveySettingsService.question_findById(question.getId()).getPage().getSurveyDefinition().getDepartment().getId(),
							user)) {
				log.warn("Unauthorized access to url path " + httpServletRequest.getPathInfo() + " attempted by user login:" + principal.getName() + "from IP:"
						+ httpServletRequest.getLocalAddr());
				return "accessDenied";
			...
			if (proceed != null) {
				...
				for (int i = 0; i < question.getColumnLabelsList().size(); i++) {
					if (question.getColumnLabelsList().get(i).getLabel() != null && question.getColumnLabelsList().get(i).getLabel().trim().length() > 0) {
						...
						if (question.getColumnLabelsList().get(i).getLabel().trim().length() == 0 || question.getColumnLabelsList().get(i).getLabel().trim().length() > 75) {
