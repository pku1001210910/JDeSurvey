...
public class QuestionStatisticDAOImp implements QuestionStatisticDAO {
	...
	/**
	 * Returns frequency statistics for a choice question' answers (Value,
	 * Count)
	 * 
	 * @param question
	 * @return
	...
	private List<QuestionStatistic> getFrequencyStatistics(Question question, final Long totalRecordCount) {
		...
		List<QuestionStatistic> questionStatistics = this.jdbcTemplate.query(selectSQLStatement, new RowMapper<QuestionStatistic>() {
	...
	/**
	 * Returns frequency statistics for a choice matrix question' answers
	 * (Value, Count)
	 * 
	 * @param question
	 * @return
	...
	private List<QuestionStatistic> getMatrixFrequencyStatistics(Question question, final Long totalRecordCount) {
		...
		for (QuestionRowLabel row : question.getRowLabels()) {
			for (QuestionColumnLabel column : question.getColumnLabels()) {
				...
				stringBuilder.append(" group by d." + columnName);
				String selectSQLStatement = stringBuilder.toString();
				List<QuestionStatistic> questionCellStatistics = this.jdbcTemplate.query(selectSQLStatement, new RowMapper<QuestionStatistic>() {
					public QuestionStatistic mapRow(ResultSet rs, int rowNum) throws SQLException {
						QuestionStatistic questionStatistic = new QuestionStatistic();
	...
	/**
	 * Returns descriptive statistics for numeric date answers (minimum,
	 * maximum)
	 * 
	 * @param question
	 * @return
	...
	private List<QuestionStatistic> getDescriptiveStatistics(Question question, final Long totalRecordCount) {
		...
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder
				.append("select MIN(d." + columnName + ") as min ,MAX(d." + columnName + ") as max ,AVG(d." + columnName + ") as avg ,STDDEV_SAMP(d." + columnName + ") as std ");
		stringBuilder.append(" from survey_data_" + surveyDefinitionId + " d inner join survey s on (s.id=d.survey_id and s.status='S')");
		String selectSQLStatement = stringBuilder.toString();
	...
	/**
	 * Returns descriptive statistics for numeric matrix question' answers
		...
		for (QuestionRowLabel row : question.getRowLabels()) {
			for (QuestionColumnLabel column : question.getColumnLabels()) {
				...
				stringBuilder.append(
						"select MIN(d." + columnName + ") as min ,MAX(d." + columnName + ") as max ,AVG(d." + columnName + ") as avg ,STDDEV_SAMP(d." + columnName + ") as std ");
				stringBuilder.append(" from survey_data_" + surveyDefinitionId + " d inner join survey s on (s.id=d.survey_id and s.status='S')");
				String selectSQLStatement = stringBuilder.toString();
				List<QuestionStatistic> questionCellStatistics = this.jdbcTemplate.query(selectSQLStatement, new RowMapper<QuestionStatistic>() {
					public QuestionStatistic mapRow(ResultSet rs, int rowNum) throws SQLException {
