...
public class SurveyPage implements Comparable<SurveyPage>, Serializable {
	...
	public SurveyPage(Survey survey, SurveyDefinitionPage surveyDefinitionPage) {
		...
		// surveyDefinitionPage.getVisibilityExpression();
	 ...
	 * this.order=surveyPage.getOrder(); this.title = surveyPage.getTitle();
	 * this.instructions = surveyPage.getInstructions();
	 * this.visibilityExpression = surveyPage.getVisibilityExpression(); for
	 * (int i = 0; i < this.questionAnswers.size() ;i++){
	 * this.questionAnswers.get(i).UpdateSettings(surveyPage.questionAnswers.get
	 * (i)); } }
	 */
	...
	private String computeMvelExpressionForSignleValueQuestion(QuestionAnswer questionAnswer, LogicalCondition condition, String groupingOperator) {
		if (condition.getStringValues() != null && condition.getStringValues().size() > 0) {
			...
			stringBuilder.setLength(stringBuilder.length() - 2); // remove the
																	// extra ||
			stringBuilder.append(") ");
	...
	private String computeMvelExpressionForMultipleValueQuestion(QuestionAnswer questionAnswer, LogicalCondition condition, String groupingOperator) {
		if (condition.getIntegerValues() != null && condition.getIntegerValues().size() > 0 && questionAnswer.getIntegerAnswerValuesArray() != null
	...
	private String computeMvelExpressionForDateAnswerQuestion(QuestionAnswer questionAnswer, LogicalCondition condition, String groupingOperator, String dateFormat) {
		...
		if (condition.getDateMin() != null && condition.getDateMax() != null) {
			...
			stringBuilder.append("page.questionAnswers[" + (questionAnswer.getOrder() - 1) + "].dateAnswerValue >="
					+ "org.apache.commons.validator.routines.DateValidator.getInstance().validate('" + DateValidator.getInstance().format(condition.getDateMin(), dateFormat)
					+ "','" + dateFormat + "')");
			stringBuilder.append(" && ");
			stringBuilder.append("page.questionAnswers[" + (questionAnswer.getOrder() - 1) + "].dateAnswerValue <="
					+ "org.apache.commons.validator.routines.DateValidator.getInstance().validate('" + DateValidator.getInstance().format(condition.getDateMax(), dateFormat)
					+ "','" + dateFormat + "')");
			stringBuilder.append(")");
			stringBuilder.append(groupingOperator);
			return stringBuilder.toString();
