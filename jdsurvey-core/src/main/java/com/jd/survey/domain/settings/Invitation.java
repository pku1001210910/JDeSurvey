...
@NamedQueries({ @NamedQuery(name = "Invitation.findSurveyAll", query = "select o from Invitation o where o.surveyDefinition.id = ?1 order by o.invitationEmailSentDate desc "),
...
public class Invitation implements Serializable, Comparable<Invitation> {
	...
	public Invitation(String firstName, String middleName, String lastName, String email, SurveyDefinition surveyDefinition) {
		super();
		this.uuid = UUID.randomUUID().toString().replace("-", "");
		this.invitationEmailSentDate = new Date();
