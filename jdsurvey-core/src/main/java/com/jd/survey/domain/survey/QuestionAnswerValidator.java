...
public class QuestionAnswerValidator implements Validator {
	...
	public QuestionAnswerValidator(String dateFormat, String validcontentTypes, String validImageTypes, Integer maximunFileSize, String invalidContentMessage,
			...
			if (question.getVisible()) {
				...
				case SHORT_TEXT_INPUT: // Short Text Input
					...
					if (valueToValidate != null && !valueToValidate.isEmpty()) {
						// validate range
						if (question.getIntegerMinimum() != null && !GenericValidator.minLength(valueToValidate, question.getIntegerMinimum())) {
							errors.rejectValue(validationFieldName, "field_length_min", new Object[] { question.getIntegerMinimum() },
									"The length of the text must exceed " + question.getIntegerMinimum());
							break;
						}
						if (question.getIntegerMaximum() != null && !GenericValidator.maxLength(valueToValidate, question.getIntegerMaximum())) {
							errors.rejectValue(validationFieldName, "field_length_max", new Object[] { question.getIntegerMaximum() },
				...
				case INTEGER_INPUT: // Integer Input
					...
					if (valueToValidate != null && !valueToValidate.isEmpty()) {
						...
						// validate range
						if (question.getIntegerMinimum() != null && !GenericValidator.minValue((int) Integer.parseInt(valueToValidate), (int) question.getIntegerMinimum())) {

							errors.rejectValue(validationFieldName, "field_value_min", new Object[] { question.getIntegerMinimum() },
									"The value of this field must exceed " + question.getIntegerMinimum());
							break;
						}
						if (question.getIntegerMaximum() != null && !GenericValidator.maxValue((int) Integer.parseInt(valueToValidate), (int) question.getIntegerMaximum())) {
							errors.rejectValue(validationFieldName, "field_value_max", new Object[] { question.getIntegerMaximum() },
				...
				case CURRENCY_INPUT: // Currency Input
					...
					if (valueToValidate != null && !valueToValidate.isEmpty()) {
						...
						// removing all '$' and ',' from string prior to
						...
						if (question.getDecimalMinimum() != null
							...
							errors.rejectValue(validationFieldName, "field_value_min", new Object[] { question.getDecimalMinimum() },
						...
						if (question.getDecimalMaximum() != null
							...
							errors.rejectValue(validationFieldName, "field_value_max", new Object[] { question.getDecimalMaximum() },
						...
						questionAnswer.setStringAnswerValue(
								currencyValidator.format(currencyValidator.validate(valueToValidate, LocaleContextHolder.getLocale()), LocaleContextHolder.getLocale()));
				...
				case DECIMAL_INPUT: // Numeric Input (Decimal)
					...
					if (valueToValidate != null && !valueToValidate.isEmpty()) {
						if (!bigDecimalValidator.isValid(valueToValidate, LocaleContextHolder.getLocale())) {
							...
							questionAnswer.setStringAnswerValue(
									bigDecimalValidator.format(bigDecimalValidator.validate(valueToValidate, LocaleContextHolder.getLocale()), LocaleContextHolder.getLocale()));
						}

						// removing all commas from string prior to validating
						// max and min
						valueToValidate = valueToValidate.replaceAll(",", "");
						// validate range
						if (question.getDecimalMinimum() != null
					...
					questionAnswer
							.setBigDecimalAnswerValue(valueToValidate.trim().length() > 0 ? bigDecimalValidator.validate(valueToValidate, LocaleContextHolder.getLocale()) : null);
					break;
				case DATE_INPUT: // Date Input
					...
					if (valueToValidate != null && !valueToValidate.isEmpty()) {
						...
						if (question.getDateMinimum() != null && (DateValidator.getInstance().validate(valueToValidate).compareTo(question.getDateMinimum()) <= 0)) {
							errors.rejectValue(validationFieldName, "field_date_min", new Object[] { question.getDateMinimum() },
									"The date entered must be after " + question.getDateMinimum());
							break;
						}
						if (question.getDateMaximum() != null && (DateValidator.getInstance().validate(valueToValidate).compareTo(question.getDateMaximum()) >= 0)) {
							errors.rejectValue(validationFieldName, "field_date_max", new Object[] { question.getDateMaximum() },
				...
				case MULTIPLE_CHOICE_CHECKBOXES: // Multiple Choice Checkboxes
					...
					if (question.getRequired()) {
						for (int i = 0; i < questionAnswer.getIntegerAnswerValuesArray().length; i++) {
							if (questionAnswer.getIntegerAnswerValuesArray()[i] != null && questionAnswer.getIntegerAnswerValuesArray()[i] <= optionsCount) {
				...
				case SHORT_TEXT_INPUT_MATRIX:// Short Text Input Matrix
					...
					for (int r = 1; r <= rowCount; r++) {
						for (int c = 1; c <= columnCount; c++) {
							...
							// continue validation if value entered is not null
							// or empty
							if (valueToValidate != null && !valueToValidate.isEmpty()) {
								// validate range
								if (question.getIntegerMinimum() != null && !GenericValidator.minLength(valueToValidate, question.getIntegerMinimum())) {
				...
				case INTEGER_INPUT_MATRIX:// Integer Input Matrix
					...
					for (int r = 1; r <= rowCount; r++) {
						for (int c = 1; c <= columnCount; c++) {
							...
							questionAnswer.getLongAnswerValuesMatrix()[r - 1][c - 1] = (valueToValidate != null && valueToValidate.length() > 0 ? Long.parseLong(valueToValidate)
									: null);
						}
					}
					break;
				case CURRENCY_INPUT_MATRIX:// Currency Input Matrix NEED TO ADD
											// MAX
											// MIN???????????????????????????????????????????????????
					rowCount = questionAnswer.getQuestion().getRowLabels().size();
					columnCount = questionAnswer.getQuestion().getColumnLabels().size();
							...
							if (valueToValidate != null && !valueToValidate.isEmpty()) {
								...
								if (!currencyValidator.isValid(valueToValidate, LocaleContextHolder.getLocale())) {
									...
									questionAnswer.getStringAnswerValuesMatrix()[r - 1][c - 1] = currencyValidator
											.format(currencyValidator.validate(valueToValidate, LocaleContextHolder.getLocale()), LocaleContextHolder.getLocale());
								}
								// removing all '$' and ',' from string prior to
								// validating max and min
								valueToValidate = valueToValidate.replaceAll("\\$", "");
						...
						for (int c = 1; c <= columnCount; c++) {
							...
							if (valueToValidate != null && !valueToValidate.isEmpty()) {
								if (!bigDecimalValidator.isValid(valueToValidate, LocaleContextHolder.getLocale())) {
									...
									questionAnswer.setStringAnswerValue(bigDecimalValidator.format(bigDecimalValidator.validate(valueToValidate, LocaleContextHolder.getLocale()),
							...
							questionAnswer.getBigDecimalAnswerValuesMatrix()[r - 1][c - 1] = (valueToValidate.trim().length() > 0
									? bigDecimalValidator.validate(valueToValidate, LocaleContextHolder.getLocale()) : null);
				...
				case DATE_INPUT_MATRIX:// Date Input Matrix
					...
					for (int r = 1; r <= rowCount; r++) {
						for (int c = 1; c <= columnCount; c++) {
							...
							questionAnswer.getDateAnswerValuesMatrix()[r - 1][c - 1] = (valueToValidate.trim().length() > 0 ? DateValidator.getInstance().validate(valueToValidate)
									: null);
						}
					}
					break;
				...
				case FILE_UPLOAD:
					...
					if (question.getRequired() && (questionAnswer.getSurveyDocument() == null) && !questionAnswer.getDocumentAlreadyUploded()) {
						errors.rejectValue(validationFieldName, "field_required", "This field is required");
						break;
					}
