...
public class SurveyService {
			...
			if (surveyPage.getOrder() < questionPageOrder) {
				for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
					...
					case YES_NO_DROPDOWN:
						questionText = questionText.replace("${p" + surveyPage.getOrder() + "q" + questionAnswer.getQuestion().getOrder() + "}",
								(questionAnswer.getStringAnswerValue() == null ? "" : questionAnswer.getStringAnswerValue()));
						break;
	...
	public SurveyPage surveyPage_get(Long surveyId, Short pageOrder, final String dateFormat) {
		...
		for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
			/*
			 * if (questionAnswer.getQuestion().getType() ==
			 * QuestionType.DATASET_DROP_DOWN){ DataSet dataset =
			 * dataSetDAO.findByName(questionAnswer.getQuestion().getDataSetCode
			 * ()); questionAnswer.getQuestion().setDataSetItems(dataSetItemDAO.
			 * findByDataSetId(dataset.getId())); continue; }
			 */
	...
	public List<SurveyPage> surveyPage_getAll(Long surveyId, final String dateFormat) {
		...
		for (SurveyPage surveyPage : surveyPages) {
			for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
				 ...
				 * if (questionAnswer.getQuestion().getType() ==
				 * QuestionType.DATASET_DROP_DOWN){ DataSet dataset =
				 * dataSetDAO.findByName(questionAnswer.getQuestion().
				 * getDataSetCode());
				 * questionAnswer.getQuestion().setDataSetItems(dataSetItemDAO.
				 * findByDataSetId(dataset.getId())); continue; }
				 */
	...
	public void surveyPage_update(SurveyPage surveyPage, String dateFormat) {
		...
		// checks if there is any branching logic on this page and update the
		// visibility of following pages if necessary.
	...
	public SurveyPage surveyPage_updateSettings(SurveyPage surveyPage) {
		...
		for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
			questionAnswer.setQuestion(questionDAO.findByOrder(surveyPage.getSurvey().getTypeId(), surveyPage.getOrder(), questionAnswer.getOrder()));
