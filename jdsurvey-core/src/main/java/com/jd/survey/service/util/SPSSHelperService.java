...
public class SPSSHelperService {
	...
	private StringBuffer getSPSSVariableName(Question question, QuestionRowLabel questionRowLabel, QuestionColumnLabel questionColumnLabel) {
		return new StringBuffer()
				.append("P" + question.getPage().getOrder() + "Q" + question.getOrder() + "R" + questionRowLabel.getOrder() + "C" + questionColumnLabel.getOrder());
	}
	...
	private String getSPSSVariableLabel(Question question) {
		String name = new StringBuffer().append("P" + question.getPage().getOrder().toString().replace("\"", "\"\"") + "-" + question.getQuestionText().replace("\"", "\"\""))
				.toString();
		if (name.length() > 60) {
	...
	private String getSPSSVariableLabel(QuestionOption option) {
		String name = new StringBuffer().append("P" + option.getQuestion().getPage().getOrder().toString().replace("\"", "\"\"") + "-"
				+ option.getQuestion().getQuestionText().replace("\"", "\"\"") + "[" + option.getText().replace("\"", "\"\"") + "]").toString();
		if (name.length() > 60) {
			return "\"" + name.substring(0, 60) + "\"";
		} else {
			return "\"" + name + "\"";
	...
	private String getSPSSVariableLabel(Question question, QuestionRowLabel questionRowLabel, QuestionColumnLabel questionColumnLabel) {
		String name = new StringBuffer()
				.append("P" + questionRowLabel.getQuestion().getPage().getOrder().toString().replace("\"", "\"\"") + "-" + question.getQuestionText().replace("\"", "\"\"") + "-"
						+ questionRowLabel.getLabel().replace("\"", "\"\"") + "-" + questionColumnLabel.getLabel().replace("\"", "\"\""))
				.toString();
		if (name.length() > 60) {
			return "\"" + name.substring(0, 60) + "\"";
		} else {
				...
				for (Question question : page.getQuestions()) {
					...
					case SINGLE_CHOICE_DROP_DOWN: // Single choice Drop Down
						...
						for (QuestionOption option : question.getOptions()) {
							sb.append("'").append(option.getValue().replace("'", "").replace("\"", "")).append("' ").append("'")
									.append(option.getText().replace("'", "").replace("\"", "")).append("' ");
						}
						sb.deleteCharAt(sb.length() - 1);
						sb.append(".\n");
						break;
					case MULTIPLE_CHOICE_CHECKBOXES: // Multiple Choice
														// Checkboxes
						for (QuestionOption option : question.getOptions()) {
						...
						break;
					/*
					 * case DATASET_DROP_DOWN: //DataSet Drop Down sb.append(
					 * "VALUE LABELS "
					 * ).append(getSPSSVariableName(question)).append("\n");
					 * dataset =
					 * dataSetDAO.findByName(question.getDataSetCode()); for
					 * (DataSetItem dataSetItem :dataset.getItems()) {
					 * sb.append("'").append(dataSetItem.getValue().replace("'",
					 * "").replace("\"", "")).append("' "
					 * ).append("'").append(dataSetItem.getText().replace("'",
					 * "").replace("\"", "")).append("' "); }
					 * sb.deleteCharAt(sb.length()-1); sb.append(".\n"); break;
					 */
					case SINGLE_CHOICE_RADIO_BUTTONS: // Single Choice Radio
														// Buttons
						sb.append("VALUE LABELS ").append(getSPSSVariableName(question)).append("\n");
						for (QuestionOption option : question.getOptions()) {
							sb.append("'").append(option.getValue().replace("'", "").replace("\"", "")).append("' ").append("'")
				...
				for (Question question : page.getQuestions()) {
					...
					if (question.getType().getIsMatrix()) {
						...
						for (QuestionRowLabel questionRowLabel : question.getRowLabels()) {
							// for each column
							for (QuestionColumnLabel questionColumnLabel : question.getColumnLabels()) {
								sb.append("VARIABLE LABELS ").append(getSPSSVariableName(question, questionRowLabel, questionColumnLabel)).append(" ")
										.append(getSPSSVariableLabel(question, questionRowLabel, questionColumnLabel)).append(".\n");
							}
						}
					} else {
	...
	public byte[] getSurveyDefinitionSPSSData(Long surveyDefinitionId) {
		try {
			...
			for (Map<String, Object> record : surveys) {
				...
				// "\"\"") +"\"," );
				// dataStringBuffer.append(record.get("last_update_date") ==
				// null ? "" : "\"" +
				// record.get("last_update_date").toString().replace("\"",
				// "\"\"") +"\"," );
				...
				dataStringBuffer.deleteCharAt(dataStringBuffer.length() - 1); // delete
																				// the
																				// last
																				// comma
