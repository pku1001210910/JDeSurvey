...
public class SurveySettingsService {
	...
	public boolean question_ValidateMinMaxValues(Question question) {
		if (question.getType() == QuestionType.INTEGER_INPUT || question.getType() == QuestionType.SHORT_TEXT_INPUT || question.getType() == QuestionType.LONG_TEXT_INPUT
				|| question.getType() == QuestionType.HUGE_TEXT_INPUT) {
			if (question.getIntegerMinimum() != null && question.getIntegerMaximum() != null && question.getIntegerMinimum() >= question.getIntegerMaximum()) {
				return false;
			}
		}
		return true;
	...
	public boolean question_ValidateDateRange(Question question) {
		if (question.getDateMinimum() != null && question.getDateMaximum() != null && question.getDateMinimum().after(question.getDateMaximum())) {
			return false;
		}
		return true;
	}
	...
	// @Transactional(readOnly = false)
	// public SurveyDefinition surveyDefinition_merge(SurveyDefinition
	// surveyDefinition) {
	...
	public Boolean surveyDefinition_ValidateNameIsUnique(SurveyDefinition surveyDefinition) throws DataAccessException {
		...
		// Insert case null id
		if (surveyDefinition.getId() == null) {
			// check that there are not survey types with the same name under
		...
		// Update case id not null
			...
			// check that there are no other survey types with the same name
			// under the current department
			for (SurveyDefinition appType : surveyDefinitions) {
				if (appType.getDepartment().getId().equals(surveyDefinition.getDepartment().getId()) && !appType.getId().equals(surveyDefinition.getId())) {
					isValid = false;
					break;
				}
			}
			...
			for (Question question : page.getQuestions()) {
				...
				if (question.getType().getRequiresOptions()) {
					if (question.getOptions().isEmpty()) {
						return false; // Multiple choice question with no
										// options
					}
				}
				// Matrix Questions
	...
	public void sendEmailReminder(SurveyDefinition surveyDefinition) {
		try {
			...
			for (User user : surveyDefinition.getUsers()) {
				...
				String emailContent = "";
				Invitation invitation = new Invitation(user.getFirstName().trim(), user.getMiddleName().trim(), user.getLastName().trim(), user.getEmail().trim(),
						surveyDefinition);
				...
				// full name
				model.put(messageSource.getMessage(INVITEE_FULLNAME_PARAMETER_NAME, null, LocaleContextHolder.getLocale()).replace("${", "").replace("}", ""), user.getFullName());
				// survey link
				...
				if (emailContent.length() > 14 && emailContent.substring(emailContent.length() - 14).toUpperCase().equalsIgnoreCase("</BODY></HTML>")) {
					...
					// template is incorrect or not html do not include tracking
					// white gif
					// emailContent = emailContent + "<img src='" +
					// trackingImageLink + invitation.getUuid() +
					// "'/></BODY></HTML>";
				}
	...
	public Question question_updateOptions(Question question) {
		...
		for (QuestionOption option : question.getOptionsList2()) {
			...
			if (option.getValue() != null && option.getText() != null && option.getValue().trim().length() > 0 && option.getText().trim().length() > 0) {
	...
	/*
	 * public DataSet dataset_findByCode(String code) { return
	 * dataSetDAO.findByCode(code); }
	 */
	...
	public boolean surveyTemplate_ValidateNameIsUnique(SurveyTemplate surveyTemplate) {
		...
		// Update case id not null
			...
			// under the current department
			for (SurveyTemplate appType : surveyTemplates) {
				if (appType.getSector().getId().equals(surveyTemplate.getSector().getId()) && !appType.getId().equals(surveyTemplate.getId())) {
					isValid = false;
					break;
				}
			}
