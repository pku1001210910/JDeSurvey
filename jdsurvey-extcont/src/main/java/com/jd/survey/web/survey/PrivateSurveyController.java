...
public class PrivateSurveyController {
	...
	/**
	 * Shows the survey submissions page if not entries found it will create a
	 * new one. if only one entry found will redirect to the survey show page
	 * 
		...
		try {
			...
			// entries found
				...
				if (userSurveyEntries.size() == 1) {
					...
					Survey survey = it.next(); // get the first and only element
												// in the set
	...
	public String submitSurvey(@RequestParam(value = "id", required = true) Long surveyId,
		...
		try {
			...
			if (proceedAction != null) { // submit button
				...
				return "redirect:/private/" + encodeUrlPathSegment(survey.getTypeId().toString(), httpServletRequest)
						+ "?list";
			} else {
				uiModel.asMap().clear();
				Survey survey = surveyService.survey_findById(surveyId);
