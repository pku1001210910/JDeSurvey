...
public class PublicSurveyController {
	...
	public void getSurveyLogo(@PathVariable("id") Long surveyDefinitionId, Model uiModel, Principal principal, HttpServletRequest httpServletRequest,
		...
		try {
			...
			if (!surveyDefinition.getIsPublic()) {// survey definition not open
													// to the public
				// attempt to access a private survey definition from a public
				// open url
	...
	/**
	 * Checks that the user does not have a pending survey entry otherwise
	 * creates a new one
	 * 
	...
	public String listSurveyEntries(@PathVariable("id") Long surveyDefinitionId, Model uiModel, Principal principal, HttpServletRequest httpServletRequest) {
		...
		try {
			...
			if (surveyDefinition.getAllowMultipleSubmissions()) {
				...
				// allow multiple submissions of this
				// survey from the same client IP Address
				if (userSurveyEntries == null || userSurveyEntries.size() == 0) {
					// No User entries for this survey,
					// create a new one
					...
					if (userSurveyEntries.size() == 1) {
						...
						Survey survey = it.next(); // get the first and only
													// element in the set
				...
				// do not allow multiple submissions of this survey from the
				...
				if (userSurveyEntries == null || userSurveyEntries.size() == 0) {
					...
					if (userSurveyEntries.size() == 1) {
						...
						Survey survey = it.next(); // get the first and only
													// element in the set
						if (survey.getStatus() == SurveyStatus.I || survey.getStatus() == SurveyStatus.R) { // survey
																											// is
																											// incomplete
																											// or
																											// reopened
	...
	public String createSurvey(@PathVariable("id") Long surveyDefinitionId, Model uiModel, Principal principal, HttpServletRequest httpServletRequest) {
	...
	public String prepareSubmitSurvey(@PathVariable("id") Long surveyId, Model uiModel, HttpServletRequest httpServletRequest) {
	...
	public String submitSurvey(@RequestParam(value = "id", required = true) Long surveyId, @RequestParam(value = "_submit", required = false) String proceedAction, Model uiModel,
		...
		try {
			if (proceedAction != null) { // submit button
				...
				// survey definition not open to the public
				if (!surveyDefinition.getIsPublic()) {
					// attempt to access a private survey definition from a
					// public open url
					log.warn(SURVEY_NOT_PUBLIC_WARNING_MESSAGE + httpServletRequest.getPathInfo() + FROM_IP_WARNING_MESSAGE + httpServletRequest.getLocalAddr());
					return "accessDenied";
				}
				...
				if (!survey.getIpAddress().equalsIgnoreCase(httpServletRequest.getLocalAddr())) {
					log.warn(
							UNAUTHORIZED_ATTEMPT_TO_ACCESS_SURVEY_WARNING_MESSAGE + httpServletRequest.getPathInfo() + FROM_IP_WARNING_MESSAGE + httpServletRequest.getLocalAddr());
					return "accessDenied";
				}
	...
	public String editSurveyPage(@PathVariable("id") Long surveyId, @PathVariable("po") Short order, Model uiModel, HttpServletRequest httpServletRequest) {
	...
	public String updateSurveyPage(@Valid SurveyPage surveyPage, @RequestParam(value = "_back", required = false) String backAction,
			@RequestParam(value = "_proceed", required = false) String proceedAction, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
		try {
			...
			if (proceedAction != null) { // next button
				...
				while (fileNames.hasNext()) {
					...
					for (QuestionAnswer questionAnswer : surveyPage.getQuestionAnswers()) {
						if (questionAnswer.getQuestion().getId().equals(questionId) && multiPartRequest.getFile(fileName).getBytes().length > 0) {
							questionAnswer.setSurveyDocument(new SurveyDocument(survey.getId(), questionId, multiPartRequest.getFile(fileName).getName(),
									multiPartRequest.getFile(fileName).getContentType(), multiPartRequest.getFile(fileName).getBytes()));
						}
					}
				}
				...
				SurveyPageValidator validator = new SurveyPageValidator(messageSource.getMessage(DATE_FORMAT, null, LocaleContextHolder.getLocale()),
						globalSettings.getValidContentTypes(), globalSettings.getValidImageTypes(), globalSettings.getMaximunFileSize(), globalSettings.getInvalidContentMessage(),
						globalSettings.getInvalidFileSizeMessage());
				validator.validate(surveyPage, bindingResult);
				if (bindingResult.hasErrors()) {
					/*
					 * for (ObjectError err :bindingResult.getAllErrors()) {
					 * log.info("getObjectName:" + err.getObjectName() +
					 * " getCode:" + err.getCode() + " getDefaultMessage:" +
					 * err.getDefaultMessage()); log.info("toString:" +
					 * err.toString()); }
					 */
					uiModel.addAttribute("survey_base_path", "open");
					uiModel.addAttribute("survey", survey);
				...
				// get the survey pages from the database again, prvious call
				// updates visibility when there is branching logic
				...
				if (order.equals((short) 0)) {
					...
					// go to the next page
					uiModel.asMap().clear();
					return "redirect:/open/" + encodeUrlPathSegment(surveyPage.getSurvey().getId().toString(), httpServletRequest) + "/"
